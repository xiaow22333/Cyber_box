...
def remove_folder_if_exists():
    folder_path = os.path.join(os.path.expanduser('~'), 'Desktop', 'Cyber_box')
...
def choose():
    ...
    # 创建说明标签
    label = tk.Label(root, text="请输入难度(1-∞)")
    label.pack()
    # 创建输入框
    entry = tk.Entry(root)
    entry.pack()
...
def check_user_input(entry1):
    ...
    if user_input1 == end_name:
        ...
        shutil.rmtree(os.path.join(os.path.expanduser('~'), 'Desktop', 'Cyber_box'))
        root1.destroy()
        exit()
    else:
...
def windows():
    ...
    # 创建按钮，点击时检查用户输入
    button1 = tk.Button(root1, text="提交答案", command=lambda: check_user_input(entry1))
    button1.pack()
    # 创建作弊按钮
    button2 = tk.Button(root1, text="作弊一下", command=lambda: print('Ta的名字是:', end_name))
    button2.pack()
    # 运行主循环
    root1.mainloop()
